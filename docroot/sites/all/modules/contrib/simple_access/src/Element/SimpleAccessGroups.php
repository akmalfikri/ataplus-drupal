...
class SimpleAccessGroups extends FormElement {
  ...
  public static function processSimpleAccessGroups(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    foreach ($groups as $group) {
      ...
      $privilege = $element['#override_privilege'] || $group->canManageAccess();
  ...
  /**
   * Move all values aside which are not accessible so not to be included in
   * the table.
   *
   * @param $element
   *
